...
import random
import nltk
from utils.show_img import showImg
...
class DataSet(BaseDataSet):
    ...
    def tokenize(self, stat_ques_list, use_glove):
        ...
        if use_glove:
            ...
            # use 'MSK' to denote the masked words in questions.
            pretrained_emb.append(spacy_tool('MSK').vector)
        ...
        #     for word in nltk.pos_tag(words, tagset='universal'):
        #         if word[1] in pos_tag_count:
        #             count = pos_tag_count.get(word[1]) + 1
        #             pos_tag_count[word[1]] = count
        #         else:
        #             pos_tag_count.update({word[1]: 1})
        #
        #         if word[0] not in token_to_ix:
        #             token_to_ix[word[0]] = len(token_to_ix)
        #             if use_glove:
        #                 pretrained_emb.append(spacy_tool(word[0]).vector)
        #
        pretrained_emb = np.array(pretrained_emb)
        #
        # print("-------------------pos tag count-----------------------")
        # print("pos_tag_count: {}".format(pos_tag_count))
    ...
    def load_ques_ans(self, idx):
        if self.__C.RUN_MODE in ['train']:
            ...
            # Process question
            ques_ix_no_msk_iter, ques_ix_iter, mask_ix_iter = self.proc_ques(ques, self.token_to_ix, max_token=14)

            # Process answer
            ans_iter = self.proc_ans(ans, self.ans_to_ix)
            # print("ques_ix_iter: {}".format(ques_ix_iter))
            # print("mask_ix_iter: {}".format(mask_ix_iter))
            return ques_ix_no_msk_iter, ques_ix_iter, ans_iter, iid, mask_ix_iter
            ...
            return ques_ix_no_msk_iter, ques_ix_iter, np.zeros(1), iid, mask_ix_iter
    ...
    def proc_ques(self, ques, token_to_ix, max_token):
        if self.__C.MODEL in ['mlvqa'] and self.__C.RUN_MODE in ['train']:
            ques_ix = np.zeros(max_token, np.int64)
            mask_ix = np.zeros(max_token, np.int64)
            ques_ix_no_msk = np.zeros(max_token, np.int64)
            ...
            pos_msk_array = np.random.rand(len(words))
            prob = random.random()
            mask_counter = 0

            if prob < 0.6:
                for ix, word in enumerate(words_pos):
                     if word[0] in token_to_ix:
                         ques_ix_no_msk[ix] = token_to_ix[word[0]]

                         if (mask_counter <= self.__C.MASK_SUM) and \
                                 ((word[1] in ['VERB'] and pos_msk_array[ix] < self.__C.VERB_PROB) or \
                                 (word[1] in ['NOUN'] and pos_msk_array[ix] < self.__C.NOUN_PROB) or \
                                 (word[1] in ['PRON'] and pos_msk_array[ix] < self.__C.PRON_PROB_IMG) or \
                                 (word[1] in ['NUM'] and pos_msk_array[ix] < self.__C.NUM_PROB)):
                             ques_ix[ix] = token_to_ix['MSK']
                             mask_ix[ix] = token_to_ix[word[0]]
                             mask_counter += 1
                         else:
                             ques_ix[ix] = token_to_ix[word[0]]
                     else:
                         ques_ix[ix] = token_to_ix['UNK']
                         ques_ix_no_msk[ix] = token_to_ix['UNK']
                ...
                for ix, word in enumerate(words_pos):
                     if word[0] in token_to_ix:
                         ...
                         if (mask_counter <= self.__C.MASK_SUM) and \
                                 ((word[1] in ['PRON'] and pos_msk_array[ix] < self.__C.PRON_PROB_QUES) or \
                                 (word[1] in ['DET'] and pos_msk_array[ix] < self.__C.DET_PROB) or \
                                 (word[1] in ['ADP'] and pos_msk_array[ix] < self.__C.ADP_PROB) or \
                                 (word[1] in ['CONJ'] and pos_msk_array[ix] < self.__C.CONJ_PROB) or \
                                 (word[1] in ['ADV'] and pos_msk_array[ix] < self.__C.ADV_PROB) or \
                                 (word[1] in ['PRT'] and pos_msk_array[ix] < self.__C.PRT_PROB) or \
                                 (word[1] in ['ADJ'] and pos_msk_array[ix] < self.__C.ADJ_PROB)):
                     ...
                     if ix + 1 == max_token:
                         break
            return ques_ix_no_msk, ques_ix, mask_ix
        else:
            ques_ix = np.zeros(max_token, np.int64)
            mask_ix = np.zeros(max_token, np.int64)
    ...
    # # use the key to find corresponding value (index to words)
    # def get_token(self, token_to_ix, index):
    #     token = [token for (token, ix) in token_to_ix.items() if ix == index]
