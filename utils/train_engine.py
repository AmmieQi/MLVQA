...
from utils.count_paras import get_parameter_number

def train_engine(__C, dataset, dataset_eval=None):

    data_size = dataset.data_size
    token_size = dataset.token_size #20547
    ...
    for epoch in range(start_epoch, __C.MAX_EPOCH):
        ...
        for step, (
            ...
            ques_ix_no_msk_iter = ques_ix_no_msk_iter.cuda()
            ques_ix_iter = ques_ix_iter.cuda()
            ans_iter = ans_iter.cuda()
            mask_ix_iter = mask_ix_iter.cuda()
            ...
            for accu_step in range(__C.GRAD_ACCU_STEPS):
                ...
                sub_bbox_feat_iter = \
                    bbox_feat_iter[accu_step * __C.SUB_BATCH_SIZE:
                                  (accu_step + 1) * __C.SUB_BATCH_SIZE]
                sub_ques_ix_no_msk_iter = \
                    ques_ix_no_msk_iter[accu_step * __C.SUB_BATCH_SIZE:
                                 (accu_step + 1) * __C.SUB_BATCH_SIZE]
                sub_ques_ix_iter = \
                ...
                # --------------------------------------------jack use it to predict the masked token--------------------------------------------------
                sub_mask_iter = \
                    mask_ix_iter[accu_step * __C.SUB_BATCH_SIZE:
                                 (accu_step + 1) * __C.SUB_BATCH_SIZE]
                ...
                # jack use it to predict the masked token
                if __C.MODEL in ['mlvqa']:
                    loss_mask_item = [pred0, sub_mask_iter]
                    loss0 = loss_fn(loss_item[0], loss_item[1])
                    loss_msk = nn.CrossEntropyLoss(reduction=__C.LOSS_REDUCTION, ignore_index=0)
                    loss1 = loss_msk(loss_mask_item[0].transpose(1, 2), loss_mask_item[1])

                    loss = (1 - __C.BETA) * loss0 + loss1 * __C.BETA
